...
const AnimatedBackground = () => {
  ...
  const setupScrollAnimations = () => {
    ...
    createSectionTimeline("#specs", "specs", "skills", "top 70%");
    createSectionTimeline("#faq", "faq", "specs", "top 30%");
  };
  ...
  const updateKeyboardTransform = async () => {
    ...
    if (isMobile) {
      const mobileKeyCaps = allObjects.filter(
        (obj: SPEObject) => obj.name === "keycap-mobile"
      );
      mobileKeyCaps.forEach((keycap: SPEObject) => {
        keycap.visible = true;
      });
    } else {
      const desktopKeyCaps = allObjects.filter(
        (obj: SPEObject) => obj.name === "keycap-desktop"
      );
      desktopKeyCaps.forEach(async (keycap: SPEObject, idx: number) => {
        await sleep(idx * 70);
        keycap.visible = true;
      });
    }

    keycaps.forEach(async (keycap: SPEObject, idx: number) => {
  ...
  // Handle keyboard text visibility — always dark theme for KERO
  useEffect(() => {
    if (!splineApp) return;
    ...
    if (activeSection !== "skills") {
      ...
      // KERO is always dark theme — show dark-theme text variants
      isMobile
  ...
  useEffect(() => {
    ...
    const manageAnimations = async () => {
      ...
      // Handle Bongo Cat — appears when activeSection === "specs"
      ...
      // Handle FAQ Section Animations (teardown + floating keycaps)
